...
class CnnEncoder(tf.keras.Model):
    def __init__(self, view_ind, **kwargs):
        ...
        self.cnn_layers = [
            Conv2D(filters=8, kernel_size=(5,5), activation='relu', padding="same", kernel_regularizer=tf.keras.regularizers.L2(1e-10)), #(,28,28,8)
            MaxPool2D(pool_size=(2,2), strides=(2,2), padding="valid"),             #(,14,14,8)
            Conv2D(filters=6, kernel_size=(5,5), activation='relu', padding="same", kernel_regularizer=tf.keras.regularizers.L2(1e-10)), #(,14、,14,6)
            MaxPool2D(pool_size=(2,2), strides=(2,2), padding="valid"),             #(,7,7,6)
            Conv2D(filters=4, kernel_size=(5,5), activation='relu', padding="same", kernel_regularizer=tf.keras.regularizers.L2(1e-10)),  #(,7,7,4)
            MaxPool2D(pool_size=(2,2), strides=(2,2), padding="valid"),             #(,3,3,4)
            Conv2D(filters=2, kernel_size=(5,5), activation='relu', padding="same", kernel_regularizer=tf.keras.regularizers.L2(1e-10)),  #(,3,3,2)
            Flatten(),                                                         #(,18)                  #自相关性有震荡但是没有坍塌  #corr_max =0,37  loss_min = 1.5  accu_max = 0.85 对比各项数据，这组参数设置更好
